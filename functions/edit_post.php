		...
		<div id="content">
			<div class="row">
				...
				<?php 
					...
					$posts_run = mysqli_query($con, $posts);
					$row = mysqli_fetch_array($posts_run);
					$post_title = $row['post_title'];
					$post_content = $row['post_content'];
				?>
					<form method="POST" action="">
						<h3>Share your thoughts...</h3>
						<input type="text" name="status_topic_title" class="form-control"  value="<?php echo $post_title; ?>">
						<textarea name="status_text" id="textarea" cols="30" rows="4" class="form-control"><?php echo $post_content; ?></textarea>
						<span class="small" id="textarea_feedback"></span> <br>
						<select class="form-control" name="select_topic" id="select_topic">
							<option>Select a topic</option>
							<?php showOption(); ?>
						</select>
						<input type="submit" value="Update" name="edit_status" class="btn btn-large btn-primary">
						<?php
							if (isset($_POST['edit_status'])) {
								$post_id = $_GET['post_id'];
								$post_title = $_POST['status_topic_title'];
								$post_content = $_POST['status_text'];
								$topic = $_POST['select_topic'];

								$updatePost = "UPDATE `posts` SET `post_title` = '$post_title', `post_content`='$post_content' WHERE `post_id` = '$post_id' ";
								$updatePost_run = mysqli_query($con, $updatePost);
								if ($updatePost_run) {
