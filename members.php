		...
		<div class="row" id="head_wrap">
			<div class="col-md-7">
				<ul>
					...
					<li><a class="activeII" href="members.php">Members</a></li>
		...
		<div id="content">
			<div class="row">
				...
				<div class="col-md-9" id="timeline">
					<h4>All the members of our small community</h4>
				<?php
					$members = "SELECT * FROM `users` ";
					$members_run = mysqli_query($con, $members);
					while ($row = mysqli_fetch_array($members_run)) {
						$user_id = $row['user_id'];
						$user_image = $row['user_image'];
						$firstName = $row['user_firstname'];
						$lastName = $row['user_lastname'];
						$country = $row['user_country'];
						echo "
						  	<span class='all_users'><a href='user_profile.php?user_id=$user_id'><img src='user/user_images/$user_image' width='70' height='70' class='thumbnail' title='$firstName $lastName'></a></span>
						";
					}
				?>
